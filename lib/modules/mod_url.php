...
class URL {
	...
	public function RAWtoAR($page, $nls="") {
		...
		$find[] = "%".preg_replace("%^https?://%", "https?\\Q://", $AR->host).$AR->root."\\E(?:/-".$ARCurrent->session->id."-)?".$nls_match."\\Q".$me->path."\\E(user.edit.page.html|view.html)?%";
		$repl[] = "{arCurrentPage\\1}";
		...
		$find[] = "%(https?://)?\\Q".$AR->host.$AR->root."\\E(/-".$ARCurrent->session->id."-)?(".$nls_match.")?/%";
		$repl[] = "{arBase\\3}/";
